...
function ListBuffs(unit)
    ...
    for i = 1, 32 do
        ...
        local buffName = textleft1:GetText()
        tooltip:Hide()

        if buffName then
            DEFAULT_CHAT_FRAME:AddMessage('Buff [' .. i .. ']: ' .. buffName)
        else
            break
        end
    end
...
function ListDebuffs(unit)
    ...
    while true do
        ...
        if debuffName then
            DEFAULT_CHAT_FRAME:AddMessage('Debuff [' .. i .. ']: ' .. debuffName)
        else
            break
        end
        i = i + 1
