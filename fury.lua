...
FsR_Stuff2Track={
	["Gold"] = {itemkind = "special", collector = {"Solomonone"}},
	...
	["BOE"] = {itemkind = "itemGrp", collector = {"Solomonone"}},
...
MB_INT_spell={Rogue="Kick",Shaman="Earth Shock(Rank1)",Mage="Counterspell",Warrior="Pummel",Paladin="None",Priest="Silence",Hunter="Wyvern Sting",Druid="Bash",Warlock="Death Coil"}
...
function leo_Swing_Start()
	MB_swingTime = UnitRangedDamage("player") - castTime;
	MB_RAIS_castStart = false
	MB_RAIS_swingStart = GetTime();
	SendChatMessage("nc -loot", "GUILD")
end
MB_AimedID=""
...
function interrupt_boss()
	...
	if class=="Paladin" then
		if MB_stuncounter==MyPallyOrder() then
...
function leo_WarriorInterrupt()
	...
	if InMeleeRange() then
		...
		if not OnCooldown("Intercept") and MyRage()>=10 then
...
function leo_WarriorSnarePlayer()
	--if your target is an enemy faction player, throw down Hamstring
	if PVP() and InMeleeRange() then
		if UnitClass("target")=="Rogue" and MyRage() >= 10 and MyRage() <= 25 then
			SelfBuff("Battle Stance")
			DotCast("Rend")
		end
		if ((MyStance()==1 or MyStance()==3) and MyRage() >= 10) then
			DotCast("Hamstring")
		end
	end
end
...
function WarriorSnarePlayer()
	...
	if PVP() and InMeleeRange() then
		if MyStance()==1 or MyStance()==3 then
			DotCast("Hamstring")
			if InMeleeRange() and MyRage() < 10 then if not buffed("Hamstring","target") then cast("Bloodrage") end end
		else
			SelfBuff("Battle Stance")
			if InMeleeRange() and MyRage() >= 10 then DotCast("Hamstring") end
			if UnitClass("target")=="Rogue" and InMeleeRange() and MyRage() >= 10 then DotCast("Rend") end
			if InMeleeRange() and MyRage() < 10 then if not buffed("Hamstring","target") then cast("Bloodrage") end end
		end
	end
end
...
function CastTotem(totem)
	...
	if FindInTable(MB_totemtypes.nobuff,totem) then
		...
		Print(OnCooldown("Windfury Totem"))
...
function warrior_setup()
	...
	if IsAltKeyDown() then
		...
		if UnitName("target")~="Kurinnaxx" and IsCurrentAction(61) then
			UseAction(61)
			return
		end
	end
...
function warrior_tank_single()
	...
	if not IAmFocus() and IsAltKeyDown() and UnitName("target")=="Kurinnaxx" then
		...
		if IsCurrentAction(61) then UseAction(61) end
		return
	end
	--END TAUNT CODE
	...
	if (not UnitName("target") or not IsAlive("target") or (not UnitIsEnemy("target","player") and UnitLevel("target")>=10)) then TargetNotOnTank() end
	if UnitName("target") and ((TargetInCombat() or IAmFocus() or IsControlKeyDown())) then
		...
		cast("Shield Slam")
		cast("Revenge")
		if InMeleeRange() then SelfBuff("Defensive Stance") end
		cast("shield block")
...
function WarriorTaunt(msg)
	...
	if OnCooldown("Taunt") and not OnCooldown("Mocking Blow") and MyRage()>=10 and MyStance()==1 then
		cast("Mocking Blow")
		return
	end
  ...
  if TankTarget("Shazzrah") and not IAmFocus() then return end
	...
	if UnitName("target") and ((TargetInCombat() or IAmFocus() or IsAltKeyDown())) then
		if IAmFocus() or (TargetInCombat() and not IsAltKeyDown()) then
			...
			if MyRage()>73 then cast("Heroic Strike") end
			if InMeleeRange() and InCombat() then SelfBuff("Berserker Stance") end
			if not IsCurrentAction(61) then UseAction(61) end;
		else
			if IsCurrentAction(61) then UseAction(61) end;
...
---------- LEO WARRIOR ---------
function leo_warrior()
	--defensive cooldowns
	if IAmTarget() and InCombat() then
		if MyHealthPct()<.3 then
			if MyStance()==2 and SetGear==2 and not OnCooldown("Shield Wall") then cast("Shield Wall") end
			if not OnCooldown("Last Stand") then cast("Last Stand") end
		end
	end
	--interrupt enemy spellcasting
	if eCasting==1 then leo_warrior_interrupt() end
	---------- WARRIOR DPS ROTATION ----------
	if SetGear~=2 then
	--sit down to get Enraged
	if not buffed("Enrage","player") and IAmTarget() and InMeleeRange() and InCombat() and eSwing~=nil and eSwing <= eSwingMax*0.10 and eSwing~=0 and not (UnitClass("target")=="Priest" or UnitClass("target")=="Mage") then RunLine("/sit") end
	--cast Execute whenever possible
	if MyStance() ~= 2 and InMeleeRange() and (UnitHealth("target")/UnitHealthMax("target")<=.2) then cast("Execute") end
	--only use abilitites if I'm in combat and in melee range
	if InMeleeRange() and InCombat() then
		SelfBuff("Battle Shout")
		---------- PVP ---------
		--PVP rotation: use Retaliation, Disarm on cooldown and keep Rend, Hamstring refreshed on target
		leo_PVP()
		---------- /PVP ---------
		--cast Overpower when possible
		if IR_OVERPOWER~=nil and not OnCooldown("Overpower") then
			--if I'm not in Battle Stance then try to stance dance
			if MyStance()~=1 then
				if MyRage()<=25 then SelfBuff("Battle Stance") end
				--dump rage so I can enter Battle Stance
				if MyRage()>25 then
					if not buffed("Flurry","player") then
						cast("Hamstring")
						else cast("Bloodthirst")
							if OnCooldown("Bloodthirst") then cast("Whirlwind") end
							--if using Two Handed then I prefer to use Slam when not attacked
							if IsSetEquipped("Two Handed") then
								if not IAmTarget() then if st_timer>UnitAttackSpeed"player"*0.75 then cast("Slam") end
									else cast("Heroic Strike")
								end
								else cast("Heroic Strike")
							end
					end
			...
			--if I'm in Battle Stance use Overpower
			if MyStance()==1 and MyRage()>=5 then cast("Overpower") end
		end
		--cast Bloodthirst on cooldown
		if MyRage()>=30 and not OnCooldown("Bloodthirst") then cast("Bloodthirst") end
		--only cast Whirlwind if Bloodthirst is on cooldown
		if not OnCooldown("Whirlwind") and OnCooldown("Bloodthirst") then
			--dump rage until changing to Berserker Stance is possible
			if MyStance() ~= 3 then
				if MyRage()<=25 then SelfBuff("Berserker Stance") end
				if MyRage()>25 then
					if not buffed("Flurry","player") then
						cast("Hamstring")
						elseif IsSetEquipped("Two Handed") then
								if not IAmTarget() then if st_timer>UnitAttackSpeed"player"*0.75 then cast("Slam") end
									else cast("Heroic Strike")
								end
							else cast("Heroic Strike")
			...
			if MyStance() == 3 then
				if not OnCooldown("Berserker Rage") then SelfBuff("Berserker Rage") end
		...
		--dump rage with Hamstring if I'm not Flurry buffed, or Heroic Strike if I am
		if not buffed("Flurry","player") then
			if MyRage()>=65 then cast("Hamstring")
				elseif MyRage()>=70 then
					if IsSetEquipped("Two Handed") then
							if not IAmTarget() then if st_timer>UnitAttackSpeed"player"*0.75 then cast("Slam") end
								else cast("Heroic Strike")
							end
	...
	---------- WARRIOR DPS ROTATION end ----------
	---------- WARRIOR TANK ROTATION ----------
	if SetGear==2 then
	--- Auto TAUNT ---
	local tname=UnitName("target")
	local ttname=UnitName("targettarget")
	if tname and ttname and UnitIsEnemy("player","target") and ttname~=myname then leo_warrior_auto_taunt() end
	--- Auto TAUNT end ---
	if (not UnitName("target") or not IsAlive("target") or (not UnitIsEnemy("target","player") and UnitLevel("target")>=10)) then leo_TargetNotOnTank() end
	--- Auto TARGET end ---
	if UnitName("target") and InMeleeRange() and InCombat() and TargetInCombat() then
		SelfBuff("Battle Shout")
		leo_PVP()
		if not OnCooldown("Berserker Rage") and MyStance()~=3 and MyRage()<=25 then SelfBuff("Berserker Stance") end
		if not OnCooldown("Berserker Rage") then SelfBuff("Berserker Rage") end
		if MyStance()==2 and not OnCooldown("Bloodrage") then SelfBuff("Bloodrage") end
		--disarm dangerous enemies
		if not OnCooldown("Disarm") and MyRage()>=30 and (UnitName("target")=="Flamewaker Elite" or UnitName("target")=="Sulfuron Harbinger" or UnitName("target")=="Majordomo Executus" or UnitName("target")=="Gurubashi Axe Thrower" or UnitName("target")=="Deathknight Captain" or UnitName("target")=="Spirestone Reaver") and not buffed("Disarm","target") then cast("Disarm") end
		if not OnCooldown("Shield Block") and MyRage()>=10 then cast("Shield Block") end
		if not OnCooldown("Revenge") and MyRage()>=15 then cast("Revenge") end
		if MyRage()>=20 then DotCast("Demoralizing Shout") end
		if MyRage()>=25 then StackCast("Sunder Armor",5) end
		if not OnCooldown("Shield Slam") and MyRage()>=30 then cast("Shield Slam") end
		if MyRage()>=50 then if not buffed("Flurry","player") then
								cast("Hamstring")
								else cast("Shield Slam") cast("Heroic Strike")
							end
		end
		if MyStance()~=2 then
			if MyRage()<=25 then SelfBuff("Defensive Stance") end
			if MyRage()>25 then
					if not buffed("Flurry","player") then
						cast("Hamstring")
						else cast("Shield Slam") cast("Heroic Strike")
...
function leo_PVP()
	if PVP() then
		...
		if not buffed("Disarm","target") and not OnCooldown("Disarm") and IAmTarget() and eSwing~=0 and eSwing~=nil and (UnitClass("target")=="Rogue" or UnitClass("target")=="Warrior" or UnitClass("target")=="Paladin" or UnitClass("target")=="Hunter" or UnitClass("target")=="Shaman") then
			--stance dance rage dump
			if MyStance() ~= 2 then
				if MyRage()<=25 then SelfBuff("Defensive Stance") end
				if MyRage()>25 then
					if not buffed("Flurry","player") then
		...
		--reapply Rend and Hamstring when they fall off
		if not buffed("Rend","target") or not buffed("Hamstring","target") then
			if (MyStance()==1 or MyStance()==3) and MyRage()>=10 then DotCast("Hamstring") end
			if (MyStance()==1 or MyStance()==2) and MyRage()>=10 then DotCast("Rend") end
			--stance dance rage dump
			if MyStance()~=1 then
				if MyRage()<=25 then SelfBuff("Battle Stance") end
				if MyRage()>25 then
		...
		if MyRage()>=30 then cast("Mortal Strike") end
	end
end

function leo_warrior_interrupt()
	--interrupt enemy spellcasting
	--use Shield Bash if in Battle or Defensive Stance
	if InCombat() and InMeleeRange() then
		if (MyStance()==1 or MyStance()==2) and not OnCooldown("Shield Bash") and MyRage()>=10 then
			if not IsSetEquipped("Sword and Shield") then
				if IsSetEquipped("Dual Wield") then SetTemp=1
											   else SetTemp=3
				end
				if (SetTemp==1 or SetTemp==3) then EquipSet("Sword and Shield") end
			end
			if IsSetEquipped("Sword and Shield") and MyRage()>=10 then UnsilencedCast("Shield Bash") end
		end
		if OnCooldown("Shield Bash") and SetTemp~=nil then
				if SetTemp==1 then EquipSet("Dual Wield") SetTemp=nil
							else EquipSet("Two Handed") SetTemp=nil
				end
		end
		--use Pummel if in Berserker Stance
		if MyStance()==3 and not OnCooldown("Pummel") and MyRage()>=10 then UnsilencedCast("Pummel") end
		--use Concussion Blow when Pummel is on cooldown
		if OnCooldown("Shield Bash") and not OnCooldown("Concussion Blow") and MyRage()>=15 then UnsilencedCast("Concussion Blow") end
	end
end

function leo_TargetNotOnTank()
	...
	elseif not IsAlive("target") then TargetNearestEnemy()
	elseif not TargetInCombat() then TargetNearestEnemy()
	elseif not InMeleeRange() then TargetNearestEnemy() end
	TargetUnit("playertarget")
end

function leo_warrior_gear()
	--Equip gear based on choice
	...
	--Set 2 is Sword and Shield
	SetGear = 2
	EquipSet("Sword and Shield")
	end
...
function leo_boost()
	...
	if not OnCooldown("Recklessness") then
		if MyStance()==3 then cast("Recklessness")end
		if MyStance() ~= 3 then
			if MyRage()<=25 then SelfBuff("Berserker Stance") end
			if MyRage()>25 then
				if not buffed("Flurry","player") then
...
function leo_warrior_auto_taunt()
	if not OnCooldown("Taunt") and MyRage()>=5 then
		if MyStance()~=2 and MyRage()<=25 then SelfBuff("Defensive Stance") end
		cast("Taunt")
	end
	if OnCooldown("Taunt") and not OnCooldown("Mocking Blow") and MyRage()>=10 then
		if MyStance()~=1 and MyRage()<=25 then SelfBuff("Battle Stance") end
		cast("Mocking Blow")
	end
end

function leo_warrior_aoe()
	--only use abilitites if I'm in combat and melee range
	if (InMeleeRange() and InCombat()) then
		--sit down to get Enraged
		if not buffed("Enrage",player) and IAmTarget() and InMeleeRange() and InCombat() and eSwing <= eSwingMax*0.10 and eSwing~=0 then RunLine("/sit") end
		SelfBuff("Battle Shout")
		--check if Sweeping Strikes is available and prioritize casting it
		if not OnCooldown("Sweeping Strikes") and not IsControlKeyDown() then
			--if I'm not in Battle Stance then dump rage until changing is possible then cast Sweeping Strikes
			if MyStance()~=1 and MyRage()<=25 then SelfBuff("Battle Stance") end
			if MyStance()~=1 and MyRage()>25 then cast("Whirlwind") cast("Cleave") end
			cast("Sweeping Strikes")
				--after I cast Sweeping Strikes it's time to change to Berserker Stance or dump rage until it's possible
				else	if MyStance() ~= 3 and MyRage()<=25 then SelfBuff("Berserker Stance") end
						if MyStance() ~= 3 and MyRage()>25 then cast("Cleave") end
						--I'm checking to see if Whirlwind is available so I can use Bloodrage for rage boost
						if not OnCooldown("Whirlwind") then
							SelfBuff("Bloodrage")
							SelfBuff("Berserker Rage")
							cast("Whirlwind")
							--while Whirlwind is on cooldown I dump rage using Cleave
							else if MyRage()>=45 then cast("Cleave") end
						end
		end
		--in cases when I want quick aoe without Sweeping Strikes
		if IsControlKeyDown() then
...
function leo_warrior_charge()
	--if Control Key is not held then use Charge and Interception, otherwise shoot
	if not IsControlKeyDown() then
		if not InCombat() and not InMeleeRange() and not OnCooldown("Charge") then
			--cast Berserker Rage before charging to gain fear immunity
			if MyStance() ~= 3 and MyRage()<=25 and not OnCooldown("Berserker Rage") then SelfBuff("Berserker Stance") end
			SelfBuff("Berserker Rage")
			if MyStance() ~= 1 and MyRage()<=25 and OnCooldown("Berserker Rage") then SelfBuff("Battle Stance") end
			cast("Charge")
		end
		--in all the situations when Charge cannot be used, use Intercept when available
		if ((OnCooldown("Charge") and not InCombat()) or (CooldownTime("Charge")>2 and InCombat()) or (not OnCooldown("Charge") and MyStance() ~= 1 and MyRage()>25) or (not OnCooldown("Charge") and InCombat() )) and not InMeleeRange() and not OnCooldown("Intercept") and MyRage()>=10 then
			if MyStance() ~= 3 and MyRage()<=25 then SelfBuff("Berserker Stance") end
			SelfBuff("Berserker Rage")
			cast("Intercept")
		end
...
function leo_warrior_rend()
	if InMeleeRange() and InCombat() then
		if MyStance()==1 or MyStance()==2 then
			if MyRage() >= 10 then DotCast("Rend") end
		end
		if not buffed("Rend","target") then
			if MyRage()<=25 then SelfBuff("Battle Stance") end
			if MyRage()>25 then
				if not buffed("Flurry","player") then
					cast("Hamstring")
...
function leo_warrior_taunt()
	SelfBuff("Bloodrage")
	BuffCast("Demoralizing Shout")
	if not IAmTarget() and TargetInCombat() then SelfBuff("Defensive Stance") cast("Taunt") cast("Revenge") cast("Mocking Blow") StackCast("Sunder Armor",5) end
	if not IsCurrentAction(61) then UseAction(61) end;
end

function leo_warrior_disarm()
		if (InMeleeRange() and InCombat()) then
			if MyStance() ~= 2 and MyRage()<=25 then SelfBuff("Defensive Stance") end
			SelfBuff("Bloodrage")
			if MyRage()>=20 then cast("Disarm") end
...
function leo_warrior_slamtest()
		if false then CastSpellByName"Slam()" end
		if not _a then
			for i=1,72 do if IsAttackAction(i) then _a=i end
			end
		end
		if not IsCurrentAction(_a)then UseAction(_a)end
		if st_timer>UnitAttackSpeed"player"*0.75 then CastSpellByName"Slam()" end
end

function leo_bot_repair()
	local timer = 0
	IndicatorFrame:SetScript("OnUpdate", function()
		timer = timer + 1
		if (timer == 100) then SendChatMessage("stay", "GUILD") end
		if (timer == 200) then SendChatMessage("go %t", "GUILD") end
		if (timer == 300) then SendChatMessage("repair", "GUILD") end
		if (timer == 400) then SendChatMessage("follow", "GUILD") end
		if (timer == 500) then IndicatorFrame:SetScript("OnUpdate", nil) end
	end)
end


function leo_test()
local speed = UnitRangedDamage("target")
if speed ~= 0 then RunLine("/say ranged") else RunLine("shit") end
end

--WeaponSwap()
--PickupInventoryItem(16); if CursorHasItem() then PickupInventoryItem(17) end;
---SWORD OR BOARD---
--if InCombat() and MyHealthPct()<0.70 then RunLine("/equipoff Stoneshell Guard") end
--if InCombat() and MyHealthPct()>0.70 then RunLine("/equipoff Inventor's Focal Sword") end

function leo_bot_strategies()
	local timer = 301
	IndicatorFrame:SetScript("OnUpdate", function()
		timer = timer - 1
		if (timer == 300) then SendChatMessage("nc -loot", "GUILD") end
		if (timer == 200) then SendChatMessage("nc -gather", "GUILD") end
		if (timer == 100) then SendChatMessage("nc -quest", "GUILD") end
		if (timer == 0) then SendChatMessage("nc -reveal", "GUILD") end
		if (timer == -100) then SendChatMessage("nc -conserve mana", "GUILD") end
		if (timer == -200) then SendChatMessage("co -conserve mana", "GUILD") end
		if (timer == -300) then SendChatMessage("co +boost", "GUILD") end
		if (timer == -400) then SendChatMessage("co -cast time", "GUILD") end
		if (timer == -500) then SendChatMessage("nc ?", "GUILD") end
		if (timer == -600) then IndicatorFrame:SetScript("OnUpdate", nil) end
	end)
end

function leo_bot_revive()
	local timer = 301
	IndicatorFrame:SetScript("OnUpdate", function()
		timer = timer - 1
		if (timer == 300) then SendChatMessage("cast Resurrection", "GUILD") end
		if (timer == 200) then SendChatMessage("cast Redemption", "GUILD") end
		if (timer == 100) then IndicatorFrame:SetScript("OnUpdate", nil) end
	end)
end
---------- LEO WARRIOR ---------
...
function fury_single()
	...
	if IAmFocus() or (TargetInCombat() and not IsAltKeyDown()) then
		...
		cast("Bloodthirst")
		if OnCooldown("Bloodthirst") then cast("Whirlwind") end
		--if MyRage()<35 then cast("Cleave")
		if MyRage()>45 then cast("Heroic Strike") end
		if InMeleeRange() and InCombat() then SelfBuff("Berserker Stance") end
		if not IsCurrentAction(61) then UseAction(61) end;
	else
		if IsCurrentAction(61) then UseAction(61) end;
...
function PallyBuff()
	...
	-- Sanctuary
	-- Light
	--Print("My pally class order is "..MyPallyOrder())
	--Print("My pally group order is "..MyGroupPallyOrder())
	if InCombat() then return end
	if UnitLevel("player")==60 then
		SancDaTank()
		if MyPallyOrder()==1 then RaidBuff({"Greater Blessing of Kings"}) end
		if MyPallyOrder()==2 then RaidBuff({"Greater Blessing of Salvation"}) end
		if MyPallyOrder()==3 then RaidBuff({"Greater Blessing of Wisdom"}) end
		if MyPallyOrder()==4 then RaidBuff({"Greater Blessing of Might"}) end
		if MyPallyOrder()==5 then RaidBuff({"Greater Blessing of Light"}) end
		if MyPallyOrder()==6 then RaidBuff({"Greater Blessing of Kings"}) end
		if MyPallyOrder()==7 then RaidBuff({"Greater Blessing of Salvation"}) end
		if MyPallyOrder()==8 then RaidBuff({"Greater Blessing of Wisdom"}) end
		if MyPallyOrder()==9 then RaidBuff({"Greater Blessing of Might"}) end
		if MyPallyOrder()==10 then RaidBuff({"Greater Blessing of Light"}) end
		...
		BlessParty()
		--if MyGroupPallyOrder()==1 then SelfBuff("Devotion Aura") end
		--if MyPallyOrder()==1 then RaidBuff({"Blessing of Kings"}) end
		--if MyGroupPallyOrder()==2 then SelfBuff("Frost Resistance Aura") end
		--if MyPallyOrder()==2 then RaidBuff({"Blessing of Might"}) end
		--if MyGroupPallyOrder()==3 then SelfBuff("Concentration Aura") end
		--if MyPallyOrder()==3 then SancDaTank() end
		--if MyGroupPallyOrder()==4 then SelfBuff("Devotion Aura") end
		--if MyPallyOrder()==4 then RaidBuff({"Blessing of Kings"}) end
		--if MyGroupPallyOrder()==5 then SelfBuff("Retribution Aura") end
		--if MyPallyOrder()==5 then RaidBuff({"Blessing of Might"}) end
		--if MyGroupPallyOrder()==6 then SelfBuff("Concentration Aura") end
		--if MyPallyOrder()==6 then RaidBuff({"Blessing of Wisdom"}) end
		--if MyGroupPallyOrder()==7 then SelfBuff("Devotion Aura") end
		--if MyPallyOrder()==7 then RaidBuff({"Blessing of Kings"}) end
		--if MyGroupPallyOrder()==8 then SelfBuff("Retribution Aura") end
		--if MyPallyOrder()==8 then RaidBuff({"Blessing of Might"}) end
		--if MyGroupPallyOrder()==9 then SelfBuff("Concentration Aura") end
		--if MyPallyOrder()==9 then RaidBuff({"Blessing of Wisdom"}) end
	end
...
function shammy_enh_aoe()
	...
	--Stoneclaw()
	if not IAmFocus() then LockonTarget() end
	if not TargetInCombat() then if IsCurrentAction(61) then UseAction(61) end end
...
function PerfectAim()
	...
	if SpellExists("Aimed Shot") then
		if not MB_RAIS_shooting then UseAction(61) return end
		...
		if not IsAutoRepeatAction(72) then UseAction(61) end
	end
end
function fd()
	PetPassiveMode()
...
function hunter_single()
	if not TargetInCombat() and not IAmFocus() then if MB_RAIS_shooting then UseAction(61) end end
	GTFO()
	if ImBusy() then return end
	if IsAltKeyDown() then
...
function IAmTarget()
	if UnitName("targettarget")==myname then return true end
end
function dru_setup()
	if IsControlKeyDown() then
...
function rogue_single()
	...
	if not TankTarget("The Prophet Skeram") and IsAltKeyDown() then if IsCurrentAction(61) then UseAction(61) return end end
	GTFO()
	INT()
	if MB_do_an_interrupt then cast(MB_INT_spell[myclass]) MB_do_an_interrupt=nil end
	if buffed("Thorns","target") and UnitName("target")=="Anubisath Sentinel" then if IsCurrentAction(61) then UseAction(61) end end
	if not IAmFocus() then LockonTarget() end
	if TargetInCombat() or IAmFocus() then
		if InMeleeRange() then
...
function Stun()
	...
	for i=1,10 do
		...
		if FindInTable(MB_My_stun_target,GetRaidTargetIndex("target")) and not UnitIsDead("target") then
			if MyPallyOrder()==MB_stuncounter then
				if UnitName("target") and not buffed("Hammer of Justice","target") and not OnCooldown("Hammer of Justice") then
					--MB_msg("My stun counter is "..MB_stuncounter.." and my pally order is "..MyPallyOrder())
...
function MB_assign_cc()
	if not IAmFocus() then return end
	if not IsRaidLeader() then Print("You are expecting me to assign raid targets and I'm not raid leader--please fix.") return end
